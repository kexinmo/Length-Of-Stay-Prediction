PROC IMPORT OUT= x1 
	DATAFILE= "/home/u42024937/fall1_crm/project/health_w:id.csv" 
	DBMS=csv REPLACE;
    GETNAMES=YES;
RUN;
...
/* ordinal logistic regression - observed variables */

proc logistic data = train;
class bed_grade city_code_hospital department ward_type;
model stay = bed_grade city_code_hospital department ward_type available_extra_rooms/ link = clogit;
...
model stay = age bed_grade city_code_hospital department   
	  ...
	  available_extra_rooms visitors_with_patient / link = clogit;
score data=work.test out=work.prediction;
run; * acc = 0.355491;
